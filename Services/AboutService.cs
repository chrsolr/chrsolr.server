...
public class AboutService : IAboutService
{
    ...
    public async Task<AboutDTO> GetAboutMe()
    {
        ...
        var user = await _context
            .Users.Where(user => user.Email == "chr.solr@gmail.com")
            .Select(user => new AboutDTO
            {
                ...
                Socials = user
                    .Socials.Where(social => social.IsActive)
                    .Select(social => new SocialDTO { Name = social.Name, Url = social.Url })
                    .ToList(),
                Educations = user
                    .Educations.Select(edu => new EducationDTO
                    {
                        Order = edu.Order,
                ...
                Jobs = user
                    .Jobs.Select(job => new JobDTO
                    {
                        ...
                        Responsibilities = job.Responsibilities.Select(resp => resp.Name).ToArray()
                    })
                    .ToList()
            })
            .FirstOrDefaultAsync();

        string[] allTechnologiesUsed = user!
            .Jobs.SelectMany(j => j.Technologies.Select(j => j).ToArray())
            .ToArray();
